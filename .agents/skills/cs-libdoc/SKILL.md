...
3. **生成 `manifest.yaml`**——所有条目初始 `status: pending`；落盘后用 `validate-yaml.py --file docs/api/manifest.yaml --yaml-only` 校验语法；展示给用户 review
...
3. 更新 `manifest.yaml` 对应条目 `status: current`
...
- [ ] 用户做了整体 review
- [ ] `manifest.yaml` 所有条目 status 已同步

**Phase 3**

- [ ] outdated 条目已全部更新或确认不需更新
- [ ] `manifest.yaml` 无残留 outdated（除非用户明确暂缓）
