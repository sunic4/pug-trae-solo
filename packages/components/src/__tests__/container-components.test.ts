...
import { TopAppBar } from '../container/top-app-bar'
import { BottomNavigation } from '../container/bottom-navigation'
import { Box } from '../layout/box'
import { Modifier } from '@pug-canvas-ui/layout'
import { Text } from '../basic/text'
import { Scaffold } from '../container/scaffold'
import { TabRow } from '../container/tab-row'
import {
  hasModifierElement,
...
describe('Surface (replaces Card)', () => {
  ...
  it('should emit a surface with custom color and elevation', () => {
    ...
    expect(node.data.borderRadius).toBe(16)
  })
...
describe('Scaffold', () => {
  ...
  it('should accept backgroundColor via options', () => {
    ...
    const rootNode = ctx.emittedNodes.get(ctx.rootNodeId!)!
    assertScaffoldNodeData(rootNode.data)
    expect(rootNode.data.color).toEqual(bgColor)
  })
})
