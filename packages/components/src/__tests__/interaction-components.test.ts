...
import { Slider, sliderValueFromPosition } from '../interaction/slider'
import { TextField } from '../interaction/text-field'
...
import type { ClickableElement } from '@pug-canvas-ui/input'
import type { GestureEvent } from '@pug-canvas-ui/input'
import {
...
describe('Button', () => {
  ...
  it('should have default color in data', () => {
    ...
    Button(ctx, onClick, () => { Text(ctx, 'Test') })
    const rootNode = ctx.emittedNodes.get(ctx.rootNodeId!)!
    assertButtonNodeData(rootNode.data)
    expect(rootNode.data.color).toBeDefined()
  })
...
describe('Slider', () => {
  it('should emit a leaf node with value data', () => {
    ...
    assertSliderNodeData(node.data)
  ...
  it('should have default value range in data', () => {
    ...
    const node = ctx.emittedNodes.get(ctx.rootNodeId!)!
    assertSliderNodeData(node.data)
    expect(node.data.valueRange).toEqual([0, 1])
  })
  ...
  it('should accept custom value range', () => {
    ...
    const node = ctx.emittedNodes.get(ctx.rootNodeId!)!
    assertSliderNodeData(node.data)
    expect(node.data.valueRange).toEqual([0, 100])
  })
...
describe('TextField', () => {
  it('should emit a leaf node with value data', () => {
    ...
    const node = ctx.emittedNodes.get(ctx.rootNodeId!)!
    assertTextFieldNodeData(node.data)
  ...
  it('should have default singleLine mode in data', () => {
    ...
    const node = ctx.emittedNodes.get(ctx.rootNodeId!)!
    assertTextFieldNodeData(node.data)
    expect(node.data.singleLine).toBe(true)
  })
  ...
  it('should accept placeholder in data', () => {
    ...
    const node = ctx.emittedNodes.get(ctx.rootNodeId!)!
    assertTextFieldNodeData(node.data)
    expect(node.data.placeholder).toBe('Enter text')
  })
  ...
  it('should accept multiline mode', () => {
    ...
    const node = ctx.emittedNodes.get(ctx.rootNodeId!)!
    assertTextFieldNodeData(node.data)
    expect(node.data.singleLine).toBe(false)
  })
...
describe('Checkbox', () => {
  it('should emit a leaf node with checked state', () => {
    ...
    const node = ctx.emittedNodes.get(ctx.rootNodeId!)!
    assertCheckboxNodeData(node.data)
    expect(node.data.checked).toBe(true)
  })
