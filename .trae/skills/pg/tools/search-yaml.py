...
Usage examples:
  # Search pg/compound (learning / trick / decision / explore docs share this dir)
  python pg/tools/search-yaml.py --dir pg/compound --filter doc_type=learning --filter track=pitfall
  python pg/tools/search-yaml.py --dir pg/compound --filter doc_type=trick --filter tags~=prisma
  python pg/tools/search-yaml.py --dir pg/compound --filter doc_type=decision --filter status=active --full

  # Full-text search in body + frontmatter values
  python pg/tools/search-yaml.py --dir pg/compound --query "shadow database"

  # JSON output for AI agent consumption
  python pg/tools/search-yaml.py --dir pg/compound --filter doc_type=learning --filter track=knowledge --json

  # Sort by a frontmatter date field (works on any ISO-8601 date string, YAML date, or sortable value)
  python pg/tools/search-yaml.py --dir pg/library-docs --sort-by last_reviewed --order asc   # oldest first (stalest)
  python pg/tools/search-yaml.py --dir pg/compound --sort-by date --order desc              # newest first

  # Works on any yaml-frontmatter markdown directory
  python pg/tools/search-yaml.py --dir docs/decisions --filter status=accepted
  python pg/tools/search-yaml.py --dir content/posts --filter tags~=python --query "asyncio"
"""
