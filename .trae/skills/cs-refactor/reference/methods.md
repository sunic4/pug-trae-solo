...
scan 阶段匹配候选优化点用的方法表，design 阶段每条执行步骤要引用一个方法号。

**方法号规则**：`M-L{层}-{序号}`，如 `M-L1-01`。层的划分：

- **L1 行为等价迁移** — 大改动、有风险时用，把"改"拆成多步安全动作
- **L2 代码级重构** — Fowler 经典单次动作，单函数 / 局部改写
- **L3 结构拆分** — 组件、模块、层级级别的拆分
- **L4 性能与异步** — 行为等价但运行特征变（算复杂度 / 渲染次数 / IO 次数）

**前后端适用性**：大部分是通用的，少数前端或后端特化的在字段里标注。

**统一字段**：每条方法包含 适用 / 不适用 / 步骤 / 风险点 / 验证 / 前后端 / 配哪种 scan 项。
...
**scan 阶段**：扫到一个候选优化点，匹配到最合适的方法号填入"建议映射的方法"字段。匹配不上说明这条候选写太模糊，要重写。

**design 阶段**：每个执行步骤引用方法号后，把"步骤"字段落到本项目的具体文件 / 函数。方法库里的步骤是骨架，不是直接复制的答案。

**扩展方法库**：新方法号接着层内编号递增。新增方法时完整填齐所有字段——缺字段的方法条目不准入库，否则 scan 阶段映射不到完整信息。
